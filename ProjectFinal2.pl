...
get_free_TAs(AllTAs,TeachingSchedule,Day,Slot,Count,Final_Result):-
    ...
    get_free_helper(AllTAs,Busy_TAs,Result), 
    permutation(Result,Final_Result).
...
% Remove the assigned TAs and get the rest of free TAs
delete_list([],Result,Result).
delete_list(AssignedTAs,Free_TAs,Result):-
    AssignedTAs = [H|T],
